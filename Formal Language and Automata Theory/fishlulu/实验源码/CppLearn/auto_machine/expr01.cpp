...
void printMinDFA(DFA d)
{
  ...
  int trapFlag = false;//是否已经输出了陷阱态
  int startFlag = false;//是否已经输出了初态
  int trap_index = -1;//用于输出陷阱状态号
  ...
  while(!q.empty())
  {
    ...
    if(state_num == -1){
      ...
      if(trapFlag == false){
          ...
          cout<<'q'<<trap_index<<' ';
          cout<<'q'<<trap_index<<' ';
          cout<<'q'<<trap_index<<endl;
          final_minDfaTrans[trap_index][0] = trap_index;
          final_minDfaTrans[trap_index][1] = trap_index;
          continue;
      }
    }
...
//      {
        ...
        else
        {
          ...
          cout<<'q'<<trap_index<<' ';
          final_minDfaTrans[index_table[state_num]][j] = trap_index;//更新转移矩阵
        }
    }
    cout<<endl;//换行
  }
...
//      cout<<"q"<<i<<"->";
//      cout<<(char)(j+'0');
//      if(i == trap_index)//如果是陷阱态直接输出陷阱态
//      {
//        cout<<'q'<<trap_index<<endl;
//        continue;
//      }
//
//      if(d.trans[index_table[i]][j] != -1)
//      {
//        int tran_state = d.trans[index_table[i]][j];
//        cout<<"q"<<index_table[tran_state];
//        if(d.endStates.count(tran_state))//如果可以转移到终态
//        {
//            end_terminators[j] = 1;
//        }
//      }
