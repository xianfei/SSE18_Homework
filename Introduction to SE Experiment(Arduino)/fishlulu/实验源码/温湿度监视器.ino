...
void setup() {
    ...
    Serial.println("Type,\tstatus,\t,humidity(%),\tTemperature\t");    //串口输出”Type, status, Humidity(%), Temperature(C)”
...
void loop() {
    ...
    //LCD显示温湿度值
    lcd.setCursor(12, 0);  // lcd 光标移动到第1行某个位置
    lcd.print(DHT.humidity); // 打印湿度值
    lcd.setCursor(8, 1);// 打印温度值
    lcd.print(DHT.temperature);// lcd 光标移动到第2行某个位置
    delay(1000);
}
